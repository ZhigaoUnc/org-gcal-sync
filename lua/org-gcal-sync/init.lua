...
M.config = {
  ...
  calendar_import_targets = {},  -- calendar_id → file path for imports
  webhook_port = nil,
...
function M.setup(opts)
  ...
  vim.api.nvim_create_user_command("OrgGcalDeleteEvent", function()
    local ok, err = pcall(function()
      require("org-gcal-sync.utils").delete_current_event()
    end)
    if not ok then
      vim.notify("OrgGcalDeleteEvent error: " .. tostring(err), vim.log.levels.ERROR)
    end
  end, { desc = "Delete Google Calendar event for headline at cursor" })

  vim.api.nvim_create_user_command("OrgGcalStripIds", function(opts)
    local ok, err = pcall(function()
      require("org-gcal-sync.utils").strip_gcal_ids({ file = opts.args ~= "" and opts.args or nil })
    end)
    if not ok then
      vim.notify("OrgGcalStripIds error: " .. tostring(err), vim.log.levels.ERROR)
    end
  end, { nargs = "?", desc = "Strip GCAL_ID/GCAL_UPDATED from file (no API calls)" })

  vim.api.nvim_create_user_command("OrgGcalBatchDelete", function(opts)
    local ok, err = pcall(function()
      ...
      for _, a in ipairs(args) do
        if a:match("^keyword=") then
          keyword = a:match("^keyword=(.*)")
        elseif a:match("^file=") then
          file = a:match("^file=(.*)")
        end
      end
      utils.batch_delete_events({ keyword = keyword, file = file })
    end)
    if not ok then
      vim.notify("OrgGcalBatchDelete error: " .. tostring(err), vim.log.levels.ERROR)
    end
  end, { nargs = "*", desc = "Batch delete GCal events (keyword=DONE file=path)" })
      ...
      callback = function()
        ...
        for _, line in ipairs(lines) do
          if line:match("SCHEDULED:") or line:match("DEADLINE:") then
            -- Debounce: cancel previous pending sync, only run latest
